<template>
  <div id="singer" v-if="!$store.state.NotifyService.playingList.show">
    ...
    <div class="list" :style="{background:'#fff'}" v-if="cd!=null">
      <ul>
      ...
      <div class="list-title" :style="{background:color}">
        <p class="desc-title">简介</p>
      </div>
      <div class="singer-brief" :style="{background:color}">
  ...
  export default {
          ...
          handler: {
            ...
            ['playAsNext'](){
              that.$store.commit('addToPlayListAsNextPlay', {
                ...
                albummid: that.cd.songlist[that.menuedIndex].album.mid
              })
            },
            ['addToPlayList'](){
    ...
    computed: {
      ...
      gradientcolor: function () {
        ...
        // return '-webkit-gradient(linear, 0 0, 0 bottom, from(rgba(255, 255, 255, 0.8)), to(#eee))'
      },
      isDark: function () {
    ...
    created: function () {
      this.$store.dispatch('getCdList', this.$route.params.id).then((response) => {
        ...
        this.desc = response.data.cdlist[0].desc.replace(/<br\s*\/?\s*>/ig, ' \n ');
      })
...
<style scoped>
  * {
  ...
  .list {
    ...
    /*background:-webkit-gradient(linear, 0 0, 0 bottom, from(rgba(255, 255, 1255, 0.8)), to(#eee));*/
    z-index: 1;
    padding-bottom: 50px;
    margin-top: -5px;
    position: absolute;
