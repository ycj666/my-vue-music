<template>
  <div id="rankpage" v-if="!$store.state.NotifyService.playingList.show">
    ...
    computed: {
      color: function () {
        if (this.topListData !== null) {
          ...
          return '#' + fixed.substr(fixed.length - 6)   // 处理颜色 返回颜色的hex值
        } else {
          return '#ffffff'
        }
      },
      ...
      r: function () {
        return parseInt(this.color.slice(1, 3), 16) // 将hex值转化为rgb格式
      },
      g: function () {
