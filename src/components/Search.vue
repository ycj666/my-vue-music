<template>
  <div id="search">
    ...
    <div class="search">
      <!--<div class="menu">
        <img src="../assets/icon-menu.png" alt="个人中心" @click=''>
      </div>-->
      <div class="search-input">
        <img src="./../assets/icon-search.png" alt="搜索">
        <form @submit.prevent="search(key)">  <!--@submit.prevent 提交事件不再重载页面-->
          <input type="text"
        ...
        <img src="./../assets/icon-delete@3x.png" alt="清除" class="delete-icon" id='delete-icon' v-if="searchRes!==null" @click='clearInput()'>
      </div>
    ...
    <div class="hotkey" v-if="searchRes==null&&searchShow">
      <div class="search-history-title" v-if="searchHistory.length!==0">历史搜索</div>
    ...
    methods: {
      search: function (key) {
        ...
        this.$store.dispatch('search', key).then((response) => {
          ...
          if (index !== -1) {
            this.searchHistory.splice(index, 1)  //如果存在则删除当前项
          }
          this.searchHistory.unshift(key)  //将当前key值置于数组开头
          this.searchHistory = this.searchHistory.slice(0, 10)  //限制最多存储十个
          localStorage.searchHistory = JSON.stringify(this.searchHistory)  //将localstorage的值（字符串）转为JSON数组
        })
      },
      focus: function () {
      ...
      play: function (index) {
        let list = []
        this.searchRes.song.itemlist.forEach(item => {
          list.push({
            id: item.id,
            mid: item.mid,
            name: item.name,
...
<style scoped>
  ...
  .search-input form,.search-input input {
    width: 100%;
    height: 100%;
    border: none;
    background: #eee;
  ...
  /*.search-input form,.search-input input::-ms-input-placeholder{text-align: center;}
  .search-input form,.search-input input::-webkit-input-placeholder{text-align: center;}*/
